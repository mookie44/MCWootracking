...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
	...
	class MCWootracking {
	    ...
	    public function __construct(){
			...
			add_action( 'save_post',  array($this, 'mc_save_tracking' ), 1, 1 );
			...
			if($order->get_status()=="completed"){
			    ...
			    if(!empty(trim($mc_tracking_code))){
					if (strpos($mc_courier, 'Royal Mail') !== false && !empty($mc_tracking_code)) {
	                    ...
	                    echo "<p>Ваш заказ был отправлен Почтой России. Трек-номер: <a href='https://www.pochta.ru/tracking#{$mc_tracking_code}'>{$mc_tracking_code}</a></p>";
					}elseif (strpos($mc_courier, 'Deutschepost') !== false && !empty($mc_tracking_code)) {
				    	// check for USA and USPS
				    	$country = $order->get_shipping_country();
				    	if($country=="US"){
					    	echo "<p>Your order was despatched via {$mc_courier}. Tracking Code: <a href='https://tools.usps.com/go/TrackConfirmAction?tLabels={$mc_tracking_code}'>{$mc_tracking_code}</a></p>";
				    	}else{
					    	echo "<p>Your order was despatched via {$mc_courier}. Tracking Code: <a href='https://www.packet.deutschepost.com/web/portal-europe/packet_traceit?barcode={$mc_tracking_code}'>{$mc_tracking_code}</a></p>";
				    	}
					}else{
